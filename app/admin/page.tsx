...
export default async function AdminPage({ searchParams }: { searchParams: Promise<{ view?: string; filter?: string; q?: string; status?: string }> }) {
  ...
  const [
    submissionsResult, ordersResult, inventoryResult, paymentConfigsResult, inventoryManagerResult,
  ] = await Promise.all([
  ...
  return <CommerceShell admin><main className="admin-page container">
    ...
    {view === "settings" && <Card className="payment-settings"><div><span className="eyebrow">CHECKOUT AVAILABILITY</span><h2>Payment methods</h2><p>Authorized administrators can change customer checkout availability.</p></div><div>{(paymentConfigsResult.data ?? []).map(config => <form action={setPaymentMethodEnabled} key={config.method}><input type="hidden" name="method" value={config.method}/><input type="hidden" name="enabled" value={String(!config.is_active)}/><span className="method-icon">{config.method === "stripe_card" ? <CreditCard/> : config.method === "zelle" ? "Z" : "$"}</span><div><strong>{config.display_name}</strong><small>{config.method === "stripe_card" && !stripeReady() ? "Configuration incomplete" : config.is_active ? "Available to customers" : "Hidden"}</small></div><Button variant={config.is_active ? "outline" : "primary"} disabled={config.method === "stripe_card" && !stripeReady() && !config.is_active}>{config.is_active ? "Disable" : "Enable"}</Button></form>)}</div></Card>}
